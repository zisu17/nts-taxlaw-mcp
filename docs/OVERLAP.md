...
기존 구현은 국세청 해석례 목록만 지원한다. 본문을 읽거나 문서번호로 특정 문서를 찾는
기능은 이 서버가 맡는다.
...
이 상태를 응답에 남기면 모델이 "법령을 확인했다"고 착각하지 않는다. 층을 아예 빼면
모델은 자료가 없다고 여기거나 빈 내용을 스스로 채울 수 있다.
...
두 서버를 같은 클라이언트에 등록하는 방법은 README와 설치 가이드에 적혀 있다.
