...
지방세 문서번호는 국세 쪽과 배열이 아예 다르다. 국세는 `종류-연도-분류-일련`
(서면-2026-법규재산-0119)이지만, 지방세 유권해석은 **생산 부서명 + 일련번호**다.
실측 표본::
...
사이트의 문서번호 검색은 **일련번호 부분일치**다(`924` → `4924`·`2924` 도 매칭).
그래서 이 모듈은 조회용 일련번호를 뽑아 주는 역할까지 맡고, 동일성 판정은
:func:`is_same_local_doc_number` 가 엄격하게 한다.

설계 원칙은 국세 쪽과 같다: **없는 번호를 만들지 않는다.**
"""
...
#: 앞에 붙어 오는 라벨 — 복사할 때 자료 종류가 딸려 온다.
#:
